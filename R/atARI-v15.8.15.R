...
#'
#' @param sampling.time,
#' @param time.until.release,
#' @param time.after.release,
#' @param smooth.step.stimulus,
#' @param filter.order,
#' @param cutoff.frequency,
#' @param left.stabilisation.time,
#' @param time.rounding.digits: the parameters required to generate a normalised
#'                       ABP stimulus.
#'                       Default values: as in function
#'                       get.normalised.ABP.stimulus()
#'
#' @param T: Parámetro del modelo correspondiente al tiempo. El valor por defecto es 1.9
#' @param D: Parámetro del modelo correspondiente al factor de amortiguamiento. El valor por defecto es 0.75
#' @param K: Parámetro del modelo correspondiente a la ganancia dinámica autorreguladora. El valor por defecto es 0.9
#' @param stabilisation.time: tiempo de estabilización de la autorregulación. Por defecto el valor es 1.
#'
...
#' @param K: Parámetro del modelo correspondiente a la ganancia dinámica autorreguladora. El valor por defecto es 0.9
#'
#' @param time.instants: the time instants in which the input ABP stimulus was
#'                sampled.
#'
#' @param ABP.normalised: the normalised ABP stimulus to be used as input to the
#'                 A-T model.
#'
#' @param sampling.time: the sampling time used in the input ABP stimulus.
#'                Default value: the smallest difference between time
#'                instants (rounded to 3 decimal places - max. 1000 Hz).
#'
#' @param stabilisation.time: the time to be used for stabilisation of the model,
...
#' Obtención de los parámetros T, D y K para los ARI desde el 0 al 9.
#'
#' @return Data frame con los parámetros asociados a cada ARI.
#' @export
#'
...
#' Se generan los templates utilizados para el cálculo del ARI según el modelo de AT. Estos templates son 9
#' números enteros.
#'
#' @param sampling.time
#' @param time.until.release
...
#' @param cutoff.frequency
#' @param left.stabilisation.time
#' @param time.rounding.digits
#' @param stabilisation.time
#' @return Data frame con los siguientes datos:
#'        Los parámetros del modelo AT: $T, $D y $K.
#'        $time.instants: instantes de tiempo del monitoreo.
#'        $sampling.time: tiempo de muestreo.
#'        $ABP.normalised: Estímulo en la presión normalizado.
#'        $CBFV.step.response: Respuesta de la velocidad de flujo cerebral al cambio de la presión.
#'        $time.release: Instante de tiempo en que se aplica la maniobra.
#'        $ARI.value: Valor del índice ARI.
#' @export
#'
...
#' Se genera los parámetros para el modelo AT correspondientes a los valores decimales del ARI.
#' Desde el 0 hasta el 9 en intervalos de 0.1.
#'
#' @param rounding.digits: Cantidad de valores decimales a considerar.
#' @return
#' @export
#'
...
#' @param curve: a list with the A-T curve, as returned by the function
#'        get.AT.curve()
#' @param title: the title for the plot. Default value: NULL
#'
#' @return The function returns the resulting plot
#' @export
#'
...
#' @param curves: a list in which each element is a list with an A-T curve as
#'         returned by the function get.AT.curve()
#' @param label.name: the name for the legend of the plot. If NULL, no legend is
#'             added and curves are plotted in black.
#'             Default value: NULL
#' @param labels: a vector of the same length as curves, containing the label for
